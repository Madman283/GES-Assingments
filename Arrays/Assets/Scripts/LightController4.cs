    ...
    void Start()
    {
        if (SceneMaster == null)
        {
            Debug.LogErrorFormat(
                "The scene master light was not set in the inspector for (0)", this.name);
            return;
        }
        else if (SceneMaster.type != LightType.Directional)
        {
            Debug.LogErrorFormat(
                "The SceneMaster was set to a {0} rather than a Directional Light in the inspector for {1}!",
                SceneMaster.type, this.name);
            return;
        }
